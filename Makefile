...
ifeq ($(MAKE_DIST),)
  ...
  ifeq ($(wildcard Makehelp config.h),Makehelp config.h)
    Makehelp:;
    Makedep: ccdep.pl config.h sam2p_version.h; perl -x -S ./ccdep.pl --FAL=assert,no,yes,checker $(CXX)
    ...
    $(ALL_FALLBACK):; $(error run this first: ./configure --enable-gif --enable-lzw #)
  endif
else
