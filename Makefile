...
ifeq ($(MAKE_DIST),)
  ...
  Makehelp config.h: configure Makehelp.in config.h.in; ./configure --enable-gif --enable-lzw
  Makedep: ccdep.pl config.h sam2p_version.h Makehelp; perl -x -S ./ccdep.pl --FAL=assert,no,yes,checker $(CXX)
  # For some reason, with `ifeq (1,1)' instead below, make would run
  # ./configure twice at the first run.
  ifeq ($(wildcard Makehelp),Makehelp)
