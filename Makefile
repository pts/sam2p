...
# On a fresh checkout config.status doesn't exist yet, so fall back to a bare
# ./configure (which honours configure's own defaults).  On subsequent runs we
# use ./config.status --recheck so the user's original ./configure arguments
# are preserved.
RECONFIGURE = if [ -x config.status ]; then MAKE=$(MAKE) ./config.status --recheck; else ./configure; fi
ifeq ($(MAKE_DIST),)
  ...
  config.h: configure config.h.in; $(RECONFIGURE) && echo "" >>config.h
  Makehelp: configure Makehelp.in; $(RECONFIGURE)
  Makedep: ccdep.pl sam2p_version.h config.h Makehelp; rm -f Makedep && $(MAKEDEP_CMD)
